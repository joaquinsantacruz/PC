Procedure Ejercicio1b is
    ...
    Task Body Puente is
        ...
        SELECT 
            WHEN ((Pasa_camion'count > 0) and (peso + 1 <= 5)) => Accept Pasa_auto do 
                                                                    peso += 1;
                                                                end Pasa_auto;
        OR
            WHEN ((Pasa_camion'count > 0) and (peso + 2 <= 5)) => Accept Pasa_camioneta do
                                                                    peso += 1;
                                                                end Pasa_camioneta;
        OR
            WHEN (peso + 3 <= 5) => Accept Pasa_camion do
                                        peso += 1;
                                    end Pasa_camion;
        OR
            Accept Sale_auto do 
